...
def edit_company(id):
    ...
    if request.method == "POST":
        ...
        company.employee_count = int(request.form.get("employee_count") or 0)
        company.rating = int(request.form.get("rating", 3))
...
def export_excel():
    ...
    ws.append([
        "ID", "Company Name", "Booth Number", "Category", "Contact Person",
        "Phone", "WhatsApp", "Email", "Website", "Address", "Notes", "Rating",
        "Priority", "Follow Up", "Favorite", "Tags", "GST Number",
        "Company Type", "Country", "State", "City", "Postal Code",
        "LinkedIn", "Facebook", "Instagram", "Employees",
        "Annual Revenue", "Meeting Status", "Company Score", "Created At"
    ])
...
@app.route("/company/<int:id>/add-product", methods=["GET", "POST"])
def add_product(id):
    company = Company.query.get_or_404(id)
    if request.method == "POST":
        product = Product(
        ...
        try:
            ...
            db.session.rollback()
            flash(f"Error: {e}", "danger")
    return render_template("add_product.html", company=company)
...
def delete_product(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Error: {e}", "danger")
    return redirect(url_for("company", id=company_id))
...
@app.route("/company/<int:id>/add-contact", methods=["GET", "POST"])
def add_contact(id):
    company = Company.query.get_or_404(id)
    if request.method == "POST":
        contact = Contact(
        ...
        try:
            ...
            db.session.rollback()
            flash(f"Error: {e}", "danger")
    return render_template("add_contact.html", company=company)
...
@app.route("/company/<int:id>/upload-photo", methods=["GET", "POST"])
def upload_photo(id):
    company = Company.query.get_or_404(id)
    ...
    return render_template("upload_photo.html", company=company)
...
def delete_photo(id):
    ...
    try:
        filepath = os.path.join(app.config["UPLOAD_FOLDER"], photo.image)
